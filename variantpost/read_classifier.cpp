...
//classify read overlap with target locus into:
// 'A' : covering
...
//returns distance to nearest variant that is not the-already-aligned target
//if the read has the-already-aligned target, read is flagged so
//zero distance if multiallelelic
...
//classify if the read is worth further analysis based on local non-ref pattern
    ...
    seq(read_seq), mapq(mapq)
{
    ...
    else 
    {
        ...
        */
        ...
        variants = find_variants(aln_start, ref_seq, seq, base_qualities, cigar_vector, non_ref_quals);
        
        
        //check covering pattern (indel position shift considered)
        covering_ptrn = classify_cover_ptrn(
                            covering_start, covering_end,
                            lpos, pos, rpos, 
                            is_shiftable,
                            start_offset, end_offset,
...
//classify reads into 
// targets: reads with target aligned
// cadidates: reads potentially harbor target (worth further check)
// non_targets: reads against further check
//-----------------------------------------------------------------
void classify_reads
(
