...
cpdef object search_target(
    ...
    cdef vector[bool_t] is_reverse, is_control
    cdef vector[int] aln_starts, aln_ends
    
    cdef list tags = []
    cdef list temp_reads = []
    cdef int est_cov = 0
    cdef object read
    ...
    for r_obj, secondary_flag in temp_reads:
        ...
        if q_seq is not None and len(q_seq) < window_len:
            ...
            is_reverse.push_back(<bool_t>r_obj.is_reverse)
            ...
            is_control.push_back(secondary_flag)
            if get_tags_flag:
    ...
    _search_target(
        ...
        read_names, is_reverse, cigar_strings, aln_starts, aln_ends,
        read_seqs, qual_seqs, is_control, (second_bam is not None)
    )
    ...
    return (
        contig_dict, rslt.target_statuses, read_names, is_reverse, is_control, tags,
