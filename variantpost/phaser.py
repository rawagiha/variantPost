...
def _phase(contig_dict, skips, target_pos, local_thresh, match_penal, max_common_substr_len):
    
    if len(contig_dict) == 0:
        return None
    
    #contig_dict = to_tight_layout(contig_dict, target_pos)
    snvs, indels, actual_target = profile_non_refs(contig_dict, target_pos)
    
    if actual_target.is_null():
        return None

    contig_dict, snvs, indels = crop_contig(contig_dict, skips, snvs, indels, actual_target.pos)
...
def find_peak(contig_dict, target, snvs, match_penal, local_thresh, is_left): 
    ...
    if peak_score > 0.0: 
        peak_idx = [idx for idx, _score in enumerate(scores) if _score == peak_score][-1]
...
def remove_unclustered_snvs(contig_dict, target, snvs, match_penal, local_thresh):
    ...
    # closed with indels
    if len(alns[0][1][0]) != len(alns[0][1][1]) and len(alns[-1][1][0]) != len(alns[-1][1][1]):
        return None
    ...
    lt_peak_locus = find_peak(contig_dict, lt_far_indel, snvs, match_penal, local_thresh, True)
    ...
    rt_peak_locus = find_peak(contig_dict, rt_far_indel, snvs, match_penal, local_thresh, False) 
